...
from cs5243.data import find_repository_root


def assignment_paths() -> dict[str, Path]:
    root = find_repository_root(__file__)
    assignment = root / "assignments" / "A1"
    return {"root": root, "assignment": assignment, "assets": root / "assets" / "A1",
...
def asset_root(root: str | Path | None = None) -> Path:
    """Return the A1 asset directory, optionally beneath an explicit repository root."""
    return (Path(root) if root is not None else assignment_paths()["root"]) / "assets" / "A1"
